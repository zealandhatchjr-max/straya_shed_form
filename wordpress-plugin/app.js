...
function isWordPressMode() {
  return Boolean((window.SHED_QUOTE_CONFIG || {}).wordpressMode);
}

async function submitQuoteToWordPress(quote) {
  const config = window.SHED_QUOTE_CONFIG || {};
  if (!config.restUrl) {
    throw new Error("The website form endpoint is not configured.");
  }
  ...
  if (!response.ok) {
    throw new Error(data?.message || "The enquiry could not be submitted. Please try again.");
  }
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (isWordPressMode()) {
      await submitQuoteToWordPress(quote);
    } else {
